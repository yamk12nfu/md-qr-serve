...
- [ ] `fs.statSync` の `ENOENT` ハンドリング強化
  - 監視中に対象Markdownが削除/移動されたケースでも、ユーザー向けエラーメッセージを明確にしつつ安全に復帰できるようにする。

- [ ] モジュールレベル可変状態のクラス化
  - `server.ts` のモジュールスコープ状態（サーバ/監視/キャッシュ/接続管理）をクラスに集約し、責務分離とテスト容易性を高める。

- [ ] CSP の `unsafe-inline` から nonce ベースへの移行
  - インラインスクリプト/スタイル依存を見直し、nonce 付与方式へ移行して CSP を強化する。
